...
namespace AirportWebAPI.Controllers
{
    ...
    public class DeparturesController : Controller
    {
        ...
        {
            var departures = await _departureService.GetEntities();
            if (departures == null)
            {
                return NotFound();
            }
        ...
        {
            var departure = await _departureService.GetEntity(id);
            if (departure == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Post([FromBody] DepartureDto departureDto)
        {
            try
            {
                var departureToReturn = await _departureService.AddEntity(departureDto);
        ...
        public async Task<IActionResult> Put(Guid id, [FromBody] DepartureDto departureDto)
        {
            ...
            {
                departureDto.Id = id;
                await _departureService.UpdateEntity(departureDto);
                return NoContent();
            }
            catch (BadRequestException)
            {
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _departureService.DeleteEntity(id);
            }
            catch (NotFoundException)
            {
                return NotFound();
